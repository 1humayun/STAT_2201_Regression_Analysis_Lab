...
# Fit a quadratic regression model (degree = 2)
quadratic_model <- lm(Y ~ poly(X, 2, raw = TRUE))

# Generate a sequence of X values for smooth curve
X_seq <- seq(min(X), max(X), length.out = 100)  # More points for a smooth curve

# Predict Y values using the fitted quadratic model
Y_pred <- predict(quadratic_model, newdata = data.frame(X = X_seq))